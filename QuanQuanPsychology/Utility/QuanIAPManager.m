...
static NSString * const orderKey = @"order_key";
static NSString * const userIdKey = @"userId_key";
static NSString * const paymentKey = @"payment_key";
...
@interface QuanIAPManager ()<SKPaymentTransactionObserver, SKProductsRequestDelegate>

@property (nonatomic, assign) BOOL goodsRequestFinished; //判断一次请求是否完成

@property (nonatomic, strong) NSString *receipt; //交易成功后拿到的一个64编码字符串

@property (nonatomic, strong) NSString *date; //交易时间

@property (nonatomic, strong) NSString *userId; //交易人

@property (nonatomic, strong) NSString *orderID;

@end

@implementation QuanIAPManager

#pragma mark - Init
+(instancetype)sharedInstance{
    static QuanIAPManager *iapManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        iapManager = [[QuanIAPManager alloc] init];
    });
    return iapManager;
}
...
- (void)requestProductWithId:(NSString *)productId andOrderID:(NSString *)orderID{
    ...
    if (self.goodsRequestFinished) {
        ...
        [self filedWithErrorCode:IAP_FILEDCOED_LASTOREDERNOTFINISHED error:@"上次请求还未完成，请稍等"];
        NSLog(@"上次请求还未完成，请稍等");
    }
}
...
- (void)completeTransaction:(SKPaymentTransaction *)transaction {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(transactionSuccess)]){
        [self.delegate transactionSuccess];
    }
}
...
-(void)saveReceipt {
    ...
    NSString *fileName = [NSDate creatTimeCode];
    
    self.userId = USERUID;
    
    NSString *savedPath = [NSString stringWithFormat:@"%@/%@.plist", [SandBoxHelper iapReceiptPath], fileName];
    
    NSString *paymentType = [NSString stringWithFormat:@"%@",[NSNumber numberWithBool:self.livePay]];
...
-(void)sendAppStoreRequestBuyPlist:(NSString *)plistPath {
    ...
    NSString *uid = dic[userIdKey];
    NSString *packageID = dic[orderKey];
    NSString *paymentType = dic[paymentKey];
    
    if ([paymentType boolValue]) {
        
        [LiveCoursePayAPI orderCheckingliveCourseOrderID:packageID receipt:receipt callback:^(APIReturnState state, id data, NSString *message) {
            
            if (state == API_SUCCESS) {
                NSDictionary *dict = (NSDictionary *)data;
                NSNumber *flag = dict[@"flag"];
                if ([flag boolValue]) {
                    NSLog(@"live pay receipt remove");
                    [self removeReceipt];
                }
            }
        }];
        ...
        [CardPackagePayAPI orderCheckingWithUID:uid packageID:packageID receipt:receipt callback:^(APIReturnState state, id data, NSString *message) {
            if (state == API_SUCCESS) {
                NSDictionary *dict = (NSDictionary *)data;
                NSNumber *flag = dict[@"flag"];
                if ([flag boolValue]) {
                    NSLog(@"card parkage pay receipt remove");
                    [self removeReceipt];
                }
            }
        }];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(filedWithErrorCode:andError:)]) {
        switch (code) {
            ...
            case IAP_FILEDCOED_LASTOREDERNOTFINISHED:
                [self.delegate filedWithErrorCode:IAP_FILEDCOED_LASTOREDERNOTFINISHED andError:error];
                break;
