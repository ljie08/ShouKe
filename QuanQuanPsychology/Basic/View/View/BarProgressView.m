...
- (void)_setHeightRestrictionOfFrame:(CGFloat)height
{
    ...
    maskLayer.path = [UIBezierPath bezierPathWithRoundedRect:self.bounds byRoundingCorners: UIRectCornerBottomLeft | UIRectCornerTopLeft cornerRadii: CGSizeMake(_progressHeight / 2.0, _progressHeight / 2.0)].CGPath;
    
    self.progressView.layer.mask = maskLayer;
    self.progressView.layer.masksToBounds = YES;

    self.layer.cornerRadius =  _progressHeight / 2.0;
...
- (void)setProgressValue:(CGFloat)progressValue
{
    ...
    if (_progressValue == self.frame.size.width) {
        self.progressView.layer.cornerRadius = _progressHeight / 2.0;
    } 
